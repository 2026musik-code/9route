...
export default function AdminView() {
  ...
  const [activeTab, setActiveTab] = useState<
    "users" | "endpoints" | "models" | "settings"
  >("models");
  ...
  const handleSaveUserLimit = async () => {
    ...
    try {
      ...
      if (data.success) {
        setUsers(
          users.map((u) =>
            u.id === editingUser.id ? { ...u, rpdLimit: editLimit } : u,
          ),
        );
        setEditingUser(null);
      }
  ...
  const deleteModel = async (id: string) => {
    try {
      await apiFetch(`/api/v1/models/entry?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 overflow-x-auto">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Manage system endpoints, models, users, rate limits, and global
            configurations.
          </p>
        </div>
      ...
      {activeTab === "models" && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
          <div className="p-4 md:p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900/50">
            ...
            <button
              onClick={addModel}
              className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
            >
                  ...
                  models.map((model) => (
                    ...
                    >
                      <td className="px-6 py-4">
                        <code className="text-slate-700 dark:text-slate-300 font-mono text-xs font-bold">
                          {model.id}
                        </code>
                      </td>
                      <td className="px-6 py-4">
                        <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-bold px-2 py-1 rounded text-[10px] uppercase">
                          {model.owned_by}
                        </span>
                      </td>
                      <td className="px-6 py-4">
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800">
                <tr>
                  ...
                  <th className="px-6 py-4 font-bold uppercase text-[10px] tracking-wider text-center">
                    Status
                  </th>
                  <th className="px-6 py-4 font-bold uppercase text-[10px] tracking-wider text-right">
                    Actions
                  ...
                  endpoints.map((ep) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className="bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 font-bold px-2 py-1 rounded-lg text-xs">
                          Active
                        </span>
                      </td>
                      <td className="px-6 py-4">
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-indigo-700 dark:text-indigo-400 font-bold text-xs uppercase">
                            {u.name.charAt(0)}
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold px-2 py-1 rounded-lg text-[10px] uppercase">
                          {u.role}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-bold px-2 py-1 rounded-lg text-xs">
                          {u.plan}
                        </span>
                      </td>
                      <td className="px-6 py-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-transparent text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                />
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 block">
                  Default RPD (Requests/Day)
                </label>
                <input
                  type="number"
                  value={settings.rpd}
              ...
              <button
                ...
                className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 rounded-lg transition-colors text-sm"
              >
                Save Limits
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden p-5">
            ...
            <div className="space-y-4">
              <label className="flex items-center gap-3 cursor-pointer p-3 border border-slate-100 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <input
                  type="checkbox"
                  checked={settings.enforceApiKey}
                  onChange={(e) =>
                    setSettings({
                      ...settings,
                      enforceApiKey: e.target.checked,
                    })
                  }
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400">
                    Require valid Bearer token for all /api/v1 endpoints
                  </span>
                </div>
              </label>
              <label className="flex items-center gap-3 cursor-pointer p-3 border border-slate-100 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <input
                  ...
                  onChange={(e) =>
                    setSettings({ ...settings, logRequests: e.target.checked })
                  }
                  className="w-4 h-4 text-indigo-600 rounded border-slate-300 dark:border-slate-700 focus:ring-indigo-500"
                />
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400">
                    Store API request metadata for debugging
                  </span>
                </div>
              </label>
              <button
                onClick={updateSettings}
                className="w-full bg-slate-800 dark:bg-slate-800 hover:bg-slate-900 dark:hover:bg-slate-700 text-white font-bold py-2 rounded-lg transition-colors text-sm"
              >
                Update Security
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="email"
                  placeholder="Misal: ceodedi@gmail.com"
                ...
                <span className="text-xs text-slate-500 mt-1 block">
                  Kosongkan untuk menggunakan default env.
                </span>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 block">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="https://api.cloudflaremini.biz.id/v1"
                  value={settings.targetBaseUrl || ""}
                  onChange={(e) =>
                    setSettings({ ...settings, targetBaseUrl: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-transparent text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                />
                <span className="text-xs text-slate-500 mt-1 block">
                  Base URL endpoint without trailing slash (e.g.
                  https://api.cloudflaremini.biz.id/v1)
                </span>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 block">
                ...
                <input
                  type="password"
                  placeholder="sk-..."
                  value={settings.targetApiKey || ""}
                  onChange={(e) =>
                    setSettings({ ...settings, targetApiKey: e.target.value })
                  }
                  className="w-full px-3 py-2 bg-transparent text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                />
                <span className="text-xs text-slate-500 mt-1 block">
                  Secret API key used to authenticate with multiple Target
                  Providers.
                </span>
              </div>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md shadow-xl border border-slate-200 dark:border-slate-800">
            <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-4">
              Edit Limit for {editingUser.name}
            </h3>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 block">
