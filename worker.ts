...
api.get("/v1/models", async (c) => {
  ...
  if (modelsCache.length === 0) {
    try {
      const response = await fetch(`${settings.targetBaseUrl}/models`, {
        headers: { Authorization: `Bearer ${settings.targetApiKey}` },
      });
...
api.post("/v1/settings", async (c) => {
  let settings = await getKV(c, "settings", {
    rpm: 60,
    rpd: 50000,
    enforceApiKey: true,
    logRequests: true,
...
api.get("/v1/users", async (c) => {
  const users = await getKV(c, "users", [
    {
      id: "1",
      name: "Dedi Supriadi",
...
api.put("/v1/users/:id", async (c) => {
  ...
  if (body.rpdLimit) {
    let quota = await getKV(c, `quota_${id}`, {
      used: 0,
      totalLimit: body.rpdLimit,
      history: [],
      logs: [],
    });
    ...
    return c.json(
      {
        ...
        message: "GITHUB_CLIENT_ID is not configured in Cloudflare Workers.",
      },
      400,
    );
  }
...
api.get("/v1/auth/github/callback", async (c) => {
  ...
  const clientId = c.env.GITHUB_CLIENT_ID || "Ov23liIBWSRxMWALRd0m";
  const clientSecret =
    c.env.GITHUB_CLIENT_SECRET || "c0b068a554e17c098025cf9c8df96bce8cc2968f";
...
api.get("/dashboard", async (c) => {
  const userId = c.req.header("x-user-id") || "1";
  const quota = await getKV(c, `quota_${userId}`, {
    used: 0,
    totalLimit: 50000,
    history: [],
    logs: [],
...
api.all("/*", async (c, next) => {
  ...
  try {
    ...
    let quota = await getKV(c, `quota_${userId}`, {
      used: 0,
      totalLimit: limit,
      history: [],
      logs: [],
